...
public class pn_wo_append_editor extends pn_editor {

	public pn_wo_append_editor(Context context) {
		super(context);
	}
	...
	public void setContentView()
	{
        ...
        View view = App.Current.Workbench.getLayoutInflater().inflate(R.layout.pn_wo_append_editor, this, true);
	...
	public void onPrepared() {
		...
		if (this.txt_task_order_cell != null) {
			this.txt_task_order_cell.setLabelText("任务令号");
		...
		if (this.txt_issue_quantity_cell != null) {
			...
			this.txt_issue_quantity_cell.TextBox.addTextChangedListener(new TextWatcher(){
				...
				public void afterTextChanged(Editable s) {
					DataRow row = (DataRow)pn_wo_append_editor.this.txt_onhand_cell.getTag();
					if (row != null) {
						...
						pn_wo_append_editor.this.txt_surplus_cell.setContentText(App.formatNumber(surplus, "0.##"));
					}
				}
			...
			this.txt_issue_quantity_cell.Button.setOnClickListener(new OnClickListener(){
				...
				public void onClick(View v) {
					String qty = pn_wo_append_editor.this.txt_issue_quantity_cell.getContentText();
					pn_wo_append_editor.this.printLabel(qty);
				}
			});
		}
		...
		if (this.txt_surplus_cell != null) {
			...
			this.txt_surplus_cell.Button.setOnClickListener(new OnClickListener(){
				@Override
				public void onClick(View v) {
					String qty = pn_wo_append_editor.this.txt_surplus_cell.getContentText();
					pn_wo_append_editor.this.printLabel(qty);
				}
			});
		}
		...
		if (this.btn_next != null) {
			...
			this.btn_next.setOnClickListener(new OnClickListener(){
				@Override
				public void onClick(View v) {
					pn_wo_append_editor.this.next();
				}
			});
		}
		
		_code = this.Parameters.get("code", "");
		if (_code == null || _code.length() == 0) {
			this.close();
			return;
		}
		
		this.txt_task_order_cell.setContentText(_code);
		_line_id = this.Parameters.get("line_id", 0);
		if (_line_id > 0) {
			String sql = "WITH temp AS (select ROW_NUMBER() OVER(order by code,item_code) rownum,line_id from mm_wo_append_order_item ";
			sql += "WHERE store_keeper=? and code=? AND (status=N'待发料' or status=N'正在发料')) SELECT rownum FROM temp WHERE line_id=?";
			Parameters p = new Parameters().add(1, App.Current.UserID).add(2, _code).add(3, _line_id);
			Result<Long> r = App.Current.DbPortal.ExecuteScalar(this.Connector, sql, p, Long.class);
			if (r.HasError) {
				App.Current.showError(pn_wo_append_editor.this.getContext(), "没有指定任务令编号。");
				this.close();
			}
			
			if (r.Value > 0L) {
				_rownum = r.Value;
				this.loadAppendOrderItem(_rownum);
			}
		} else {
			this.close();
			return;
	...
	{
		if (_rownum >1 ) {
			this.loadAppendOrderItem(_rownum - 1);
		} else {
			App.Current.showError(pn_wo_append_editor.this.getContext(), "已经是第一条。");
		}
	}
	...
	public void next()
	{
		if (_rownum < _total) {
			...
			App.Current.showError(pn_wo_append_editor.this.getContext(), "已经是最后一条。");
		}
	}
	...
	public void loadAppendOrderItem(long index)
	{
		...
		String sql = "exec p_mm_wo_append_get_order_item ?,?,?,?";
		Parameters p  =new Parameters().add(1, App.Current.UserID).add(2, code).add(3, index);
		App.Current.DbPortal.ExecuteRecordAsync(this.Connector, sql, p, new ResultHandler<DataRow>(){
			@Override
    	    public void handleMessage(Message msg) {
				pn_wo_append_editor.this.ProgressDialog.dismiss();
				
				Result<DataRow> result = this.Value;
				if (result.HasError) {
					App.Current.showError(pn_wo_append_editor.this.getContext(), result.Error);
					return;
				}
				
				DataRow row = result.Value;
				if (row == null) {
					App.Current.showError(pn_wo_append_editor.this.getContext(), "没有数据。");
					pn_wo_append_editor.this.clearAll();
					pn_wo_append_editor.this.Header.setTitleText("工单发料(无发料任务)");
					return;
				}
				...
				if (_total > 0 ){
					pn_wo_append_editor.this.Header.setTitleText("工单发料(" + String.valueOf(_total) + "条发料任务)");
				} else {
					pn_wo_append_editor.this.Header.setTitleText("工单发料(无发料任务)");
				}
				
				pn_wo_append_editor.this.txt_task_order_cell.setTag(row);
				
				String item_code = row.getValue("item_code", "") + ", 第" + String.valueOf(_rownum) + "条";
				pn_wo_append_editor.this.txt_task_order_cell.setContentText(row.getValue("code", ""));
				pn_wo_append_editor.this.txt_item_code_cell.setContentText(item_code);
				pn_wo_append_editor.this.txt_item_name_cell.setContentText(row.getValue("item_name", ""));
				pn_wo_append_editor.this.txt_vendor_name_cell.setContentText(row.getValue("vendor_name", ""));
				pn_wo_append_editor.this.txt_date_code_cell.setContentText(row.getValue("date_code", ""));
				
				BigDecimal planned_qty = row.getValue("planned_quantity", BigDecimal.ZERO);
				...
				pn_wo_append_editor.this.txt_planned_quantity_cell.setContentText(qty_str);
				pn_wo_append_editor.this.txt_location_cell.setContentText(row.getValue("location", ""));
				
				pn_wo_append_editor.this.txt_lot_number_cell.setTag(null);
				pn_wo_append_editor.this.txt_lot_number_cell.setContentText("");
				pn_wo_append_editor.this.txt_issue_quantity_cell.setContentText("");
				pn_wo_append_editor.this.txt_onhand_cell.setContentText("");
				pn_wo_append_editor.this.txt_onhand_cell.setTag(null);
				pn_wo_append_editor.this.txt_surplus_cell.setContentText("");
			}
		});
	}
	...
	public void loadLotNumber(String lotNumber)
	{
		...
		final long order_id = row.getValue("head_id", Long.class);
		final int line_id = row.getValue("line_id", Integer.class);
		final int item_id = row.getValue("item_id", Integer.class);
		...
		App.Current.DbPortal.ExecuteRecordAsync(this.Connector, sql, p, new ResultHandler<DataRow>(){
			...
			public void handleMessage(Message msg) {
				...
				if (r.HasError) {
					App.Current.showError(pn_wo_append_editor.this.getContext(), r.Error);
					return;
				}
				
				String result = r.Value.getValue("result", "");
				if (result.length()>0) {
					App.Current.showError(pn_wo_append_editor.this.getContext(), result);
					return;
				}
				...
				pn_wo_append_editor.this.txt_vendor_name_cell.setContentText(r.Value.getValue("vendor_name", ""));
				pn_wo_append_editor.this.txt_vendor_name_cell.setTag(r.Value.getValue("vendor_id"));
				pn_wo_append_editor.this.txt_lot_number_cell.setTag(r.Value);
				pn_wo_append_editor.this.txt_lot_number_cell.setContentText(r.Value.getValue("lot_number", ""));
				pn_wo_append_editor.this.txt_date_code_cell.setContentText(r.Value.getValue("date_code", ""));
				pn_wo_append_editor.this.txt_location_cell.setContentText(r.Value.getValue("location_code", ""));
				
				pn_wo_append_editor.this.txt_issue_quantity_cell.setContentText(issue_qty);
				pn_wo_append_editor.this.txt_onhand_cell.setContentText(onhand_qty);
				pn_wo_append_editor.this.txt_onhand_cell.setTag(r.Value);
				pn_wo_append_editor.this.txt_surplus_cell.setContentText(surplus_qty);
			}
		});
	...
	public void printLabel(String quantity)
	{
		...
		final DataRow task_row = (DataRow)this.txt_task_order_cell.getTag();
		if (task_row == null) {
	...
	public void commit()
	{
		...
		final Long task_order_id = task_row.getValue("head_id", Long.class);
		final String task_order_code = this.txt_task_order_cell.getContentText();
		final int task_order_line_id = task_row.getValue("line_id", 0);
		final int organization_id = task_row.getValue("org_id", 0);
		final int location_id = lot_row.getValue("location_id", Integer.class);
		...
		App.Current.question(this.getContext(), "确定要提交吗？", new DialogInterface.OnClickListener(){
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				entry.put("code", task_order_code);
				entry.put("create_user", App.Current.UserID);
				entry.put("task_order_id", String.valueOf(task_order_id));
				//entry.put("work_order_id", row.getValue("code", ""));
				entry.put("task_order_line_id", String.valueOf(task_order_line_id));
				entry.put("organization_id", String.valueOf(organization_id));
				entry.put("item_id", String.valueOf(task_row.getValue("item_id", Integer.class)));
				entry.put("quantity", String.valueOf(issue_quantity));
				entry.put("uom_code", task_row.getValue("uom_code", ""));
				...
				entry.put("vendor_lot", task_row.getValue("vendor_lot", ""));
				//entry.put("from_warehouse_id", task_row.getValue("code", ""));
				entry.put("from_location_id", String.valueOf(location_id));
				
				//entry.put("to_warehouse_id", task_row.getValue("code", ""));
				//entry.put("to_location_id", task_row.getValue("code", ""));
				...
				Connection conn = App.Current.DbPortal.CreateConnection(pn_wo_append_editor.this.Connector);
				...
				try {
					...
					if (val != null) {
						...
						if (rs.HasError) {
							App.Current.showError(pn_wo_append_editor.this.getContext(), rs.Error);
							return;
						}
						
						App.Current.toastInfo(pn_wo_append_editor.this.getContext(), "提交成功");
						pn_wo_append_editor.this.clear();
						pn_wo_append_editor.this.loadAppendOrderItem(_rownum);
					}
				} catch (SQLException e) {
					App.Current.showInfo(pn_wo_append_editor.this.getContext(), e.getMessage());
					e.printStackTrace();
