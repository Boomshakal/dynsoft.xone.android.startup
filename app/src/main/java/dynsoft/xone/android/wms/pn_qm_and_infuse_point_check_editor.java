...
public class pn_qm_and_infuse_point_check_editor extends pn_editor {
    private View view;
    ...
    public pn_qm_and_infuse_point_check_editor(Context context) {
        super(context);
    }
    ...
    public void setContentView() {
        ...
        view = App.Current.Workbench.getLayoutInflater().inflate(R.layout.pn_qm_win_ipqc_point_check_editor_infuse, this, true);
...
//            });
        ...
        if (text_cell_7 != null) {
            text_cell_7.setLabelText("容器重量");
        }
        if (text_cell_8 != null) {
            text_cell_8.setLabelText("额容器+灌封主剂重量");
        }
        if (text_cell_9 != null) {
            text_cell_9.setLabelText("固化剂重量");
        }
        if (text_cell_10 != null) {
            text_cell_10.setLabelText("额容器+灌封主剂+固化剂重量");
        }
...
//    }
    ...
    private void loadComfirmName(final ButtonTextCell button_text_cell_4) {
        Link link = new Link("pane://x:code=pn_qm_and_infuse_point_check_record_parameter_mgr");
//        link.Parameters.add("textcell", textcell_1);
    ...
    private void loadWorkLineName(final ButtonTextCell text) {
        ...
        if (result.Value != null) {
            if (result.Value.Rows.size() == 1) {
                ...
                new AlertDialog.Builder(pn_qm_and_infuse_point_check_editor.this.getContext()).setTitle("请选择")
    ...
    private void loadItem(Integer id) {
        ...
        String sql = "exec get_qm_infuse_point_check_editor ?";
        ...
        App.Current.DbPortal.ExecuteRecordAsync("core_and", sql, p, new ResultHandler<DataRow>() {
            ...
            public void handleMessage(Message msg) {
                pn_qm_and_infuse_point_check_editor.this.ProgressDialog.dismiss();
                ...
                if (value.Value != null) {
                    ...
                    String curing_agent_weight = value.Value.getValue("curing_agent_weight", "");
                    String sum_weight = value.Value.getValue("sum_weight", "");
                    ...
                    text_cell_9.setContentText(curing_agent_weight);
                    text_cell_10.setContentText(sum_weight);

//                    if (n_actual != "" && n_standard != "") {
//                        if (Math.abs(Float.parseFloat(n_actual.trim()) - Float.parseFloat(n_standard.trim())) > 0.02) {
...
//                }
    ...
    public void commit() {
        ...
        if (TextUtils.isEmpty(button_text_cell_5.getContentText())) {
            ...
            App.Current.toastInfo(getContext(), "请输入 容器重量");
            App.Current.playSound(R.raw.error);
        } else if (!isNumeric(text_cell_8.getContentText())) {
            App.Current.toastInfo(getContext(), "请输入 额容器+灌封主剂重量");
            App.Current.playSound(R.raw.error);
        } else if (!isNumeric(text_cell_9.getContentText())) {
            App.Current.toastInfo(getContext(), "请输入 固化剂重量");
            App.Current.playSound(R.raw.error);
        } else if (!isNumeric(text_cell_10.getContentText())) {
            App.Current.toastInfo(getContext(), "请输入 额容器+灌封主剂+固化剂重量");
            App.Current.playSound(R.raw.error);
        } else {
            App.Current.question(this.getContext(), "确定要提交吗？", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ...
                    dialog.dismiss();
                    String sql = "exec p_qm_and_infuse_point_check_create ?,?,?,?,?,?,?,?,?,?";
