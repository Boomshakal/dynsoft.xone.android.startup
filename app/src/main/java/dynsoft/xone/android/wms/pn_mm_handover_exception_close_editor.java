...
public class pn_mm_handover_exception_close_editor extends pn_editor {

	public pn_mm_handover_exception_close_editor(Context context) {
		super(context);
	}
	...
	public TextCell txt_relay_date_cell;
	public TextCell txt_close_comment_cell;
	...
	public void setContentView()
	{
        ...
        View view = App.Current.Workbench.getLayoutInflater().inflate(R.layout.pn_mm_handover_exception_close_editor, this, true);
	...
	public void onPrepared() {
		...
		this.txt_relay_date_cell =(TextCell)this.findViewById(R.id.txt_relay_date_cell);
	    this.txt_close_comment_cell=(TextCell)this.findViewById(R.id.txt_close_comment_cell);
		...
		if (this.txt_relay_comment_cell != null) {
			...
			this.txt_relay_comment_cell.setReadOnly();
		}
		
		if (this.txt_relay_date_cell != null) {
			this.txt_relay_date_cell.setLabelText("回复日期");
			this.txt_relay_date_cell.setReadOnly();
		}
		...
		if (this.txt_close_comment_cell != null) {
			this.txt_close_comment_cell.setLabelText("结案说明");
			//this.txt_relay_date_cell.setReadOnly();
		}
		if (this.btn_prev != null) {
			...
			this.btn_prev.setOnClickListener(new OnClickListener(){
				...
				public void onClick(View v) {
					pn_mm_handover_exception_close_editor.this.prev();
				}
			});
		}
		...
		if (this.btn_next != null) {
			...
			this.btn_next.setOnClickListener(new OnClickListener(){
				@Override
				public void onClick(View v) {
					pn_mm_handover_exception_close_editor.this.next();
				}
			});
		}
		...
		if (_id > 0) {
			String sql = "exec p_mm_exception_close_get_row_number ?,?";
			...
			if (r.HasError) {
				App.Current.showError(pn_mm_handover_exception_close_editor.this.getContext(), "没有指定异常单据。");
				this.close();
			}
	...
	public void prev()
	{
		if (_rownum >1 ) {
			...
			App.Current.showError(pn_mm_handover_exception_close_editor.this.getContext(), "已经是第一条。");
		}
	}
	...
	public void next()
	{
		if (_rownum < _total) {
			...
			App.Current.showError(pn_mm_handover_exception_close_editor.this.getContext(), "已经是最后一条。");
		}
	}
	...
	public void loadExceptionItem(long index)
	{
		...
		String sql = "exec p_mm_exception_close_get_item ?,?";
		Parameters p  =new Parameters().add(1, App.Current.UserID).add(2, index);
		App.Current.DbPortal.ExecuteRecordAsync(this.Connector, sql, p, new ResultHandler<DataRow>(){
			@Override
    	    public void handleMessage(Message msg) {
				pn_mm_handover_exception_close_editor.this.ProgressDialog.dismiss();
				
				Result<DataRow> result = this.Value;
				if (result.HasError) {
					App.Current.showError(pn_mm_handover_exception_close_editor.this.getContext(), result.Error);
					return;
				}
				
				_order_row = result.Value;
				if (_order_row == null) {
					App.Current.showError(pn_mm_handover_exception_close_editor.this.getContext(), "没有数据。");
					pn_mm_handover_exception_close_editor.this.clearAll();
					pn_mm_handover_exception_close_editor.this.Header.setTitleText("事务异常回复");
					return;
				}
				...
				if (_total > 0 ){
					pn_mm_handover_exception_close_editor.this.Header.setTitleText("事务异常(共" + String.valueOf(_total) + "条)");
				} else {
					pn_mm_handover_exception_close_editor.this.Header.setTitleText("事务异常");
				}
				...
				String reply_comment =_order_row.getValue("reply_comment","");
				String reply_time=_order_row.getValue("reply_time","");
			    ...
			    pn_mm_handover_exception_close_editor.this.txt_order_code_cell.setContentText(order_code);  
				pn_mm_handover_exception_close_editor.this.txt_item_code_cell.setContentText(item_code);
				pn_mm_handover_exception_close_editor.this.txt_item_name_cell.setContentText(item_name);
				pn_mm_handover_exception_close_editor.this.txt_exception_remark_cell.setContentText(exception_remark);
				pn_mm_handover_exception_close_editor.this.txt_relay_comment_cell.setContentText(reply_comment);
				pn_mm_handover_exception_close_editor.this.txt_relay_date_cell.setContentText(reply_time);
	...
	public void commit()
	{
		...
		final String close_remark=this.txt_close_comment_cell.getContentText();
		final String user_name =this.txt_user_name.getContentText().trim();
        if (close_remark==null || close_remark.length()==0)
        {
         App.Current.showError(this.getContext(), "请填写结案说明后提交！");
         return;
        }
		...
		App.Current.question(this.getContext(), "确定要提交异常结案吗？", new DialogInterface.OnClickListener(){
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				String sql = "update  mm_exception_handling_list set  close_date=getdate(),close_comment =?,close_user_name=?  where   id=? ";
				Parameters p = new Parameters().add(1, close_remark).add(2,user_name).add(3, id);
				Result<Integer> r = App.Current.DbPortal.ExecuteNonQuery(pn_mm_handover_exception_close_editor.this.Connector, sql, p);
				if (r.HasError) {
					_order_row = null;
					 App.Current.showError(pn_mm_handover_exception_close_editor.this.getContext(), r.Error);
					 pn_mm_handover_exception_close_editor.this.clearAll();
					 pn_mm_handover_exception_close_editor.this.loadExceptionItem(_rownum);
					}
					if (r.Value > 0) {
						_order_row = null;
						App.Current.toastInfo(pn_mm_handover_exception_close_editor.this.getContext(), "提交成功!");
						pn_mm_handover_exception_close_editor.this.clearAll();
					    pn_mm_handover_exception_close_editor.this.loadExceptionItem(_rownum);
					}
	...
	public void clearAll()
	{
		...
		this.txt_close_comment_cell.setContentText("");
		this.txt_relay_date_cell.setContentText("");
	}
}
