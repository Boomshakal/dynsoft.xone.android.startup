...
public class pn_up_entry_editor extends pn_editor {
	...
	public void onPrepared() {
		...
		if (this.txt_issue_quantity_cell != null) {
			this.txt_issue_quantity_cell.setLabelText("实入数量");
	...
	public void prev()
	{
		if (_rownum >1 ) {
			...
			App.Current.showError(pn_up_entry_editor.this.getContext(), "已经是第一条。");
		}
	}
	...
	public void next()
	{
		if (_rownum < _total) {
			...
			App.Current.showError(pn_up_entry_editor.this.getContext(), "已经是最后一条。");
		}
	}
	...
	public void loadTaskOrderItem(long index)
	{
		...
		App.Current.DbPortal.ExecuteRecordAsync(this.Connector, sql, p, new ResultHandler<DataRow>(){
    	    ...
    	    public void handleMessage(Message msg) {
				...
				if (row == null) {
					App.Current.showError(pn_up_entry_editor.this.getContext(), "没有数据。");
					pn_up_entry_editor.this.clearAll();
					pn_up_entry_editor.this.Header.setTitleText("杂项入库");
					return;
				}
				...
				if (_total > 0 ){
					pn_up_entry_editor.this.Header.setTitleText("杂项入库(共" + String.valueOf(_total) + "条)");
				} else {
					pn_up_entry_editor.this.Header.setTitleText("杂项入库");
				}
				...
				qty_str += "/已入:" + App.formatNumber(issued_qty, "0.##");
				qty_str += "/未入:" + App.formatNumber(open_qty, "0.##");
	...
	public void loadLotNumber(String lotNumber)
	{
		...
		if (_order_row == null) {
			App.Current.showError(this.getContext(), "没有指定杂项入库，无法扫描条码。");
			return;
		}
	...
	public void printLabel(String quantity, boolean withIssueOrderCode)
	{
		...
		if (_order_row == null) {
			App.Current.showError(this.getContext(), "没有杂项入库数据。");
			return;
		}
	...
	{
		if (_order_row == null) {
			App.Current.showError(this.getContext(), "没有杂项入库数据，不能提交。");
			return;
		}
		...
		{
			App.Current.showError(this.getContext(), "没有储位数据，不能提交。");
			return;
		}
		...
		if (issue == null || issue.length() == 0) {
			App.Current.showError(this.getContext(), "没有输入库数量，不能提交。");
			return;
		}
		
		final BigDecimal issue_quantity = new BigDecimal(issue);
		if (issue_quantity.equals(BigDecimal.ZERO)){
			App.Current.showError(this.getContext(), "入库数量为0，不能提交。");
			return;
		}
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				try {
					...
					if (val != null) {
						...
						App.Current.toastInfo(pn_up_entry_editor.this.getContext(), "提交成功");
						_order_row = null;
						_lot_row = null;
