	 ...
	 protected void onCreate(Bundle savedInstanceState) {
		...
		/* if (txt_org_code_cell != null) {
		 ...
		 if (txt_uom_code_cell != null) {
			 txt_uom_code_cell.setLabelText("单位");
		 ...
		 if (btn_print != null) {
			 btn_print.setOnClickListener(new OnClickListener(){
				...
				public void onClick(View v) {
					...
					if (copies == 0) {
						App.Current.showError(frm_item_lot_printer_init.this, "无效的打印份数。");
						return;
					}
					 
					if (server == null || server.length() == 0) {
						App.Current.showError(frm_item_lot_printer_init.this, "请指定打印机服务器地址。");
						return;
					}
					
					if (printer == null || printer.length() == 0) {
						App.Current.showError(frm_item_lot_printer_init.this, "请指定打印机机名称。");
						return;
					}
