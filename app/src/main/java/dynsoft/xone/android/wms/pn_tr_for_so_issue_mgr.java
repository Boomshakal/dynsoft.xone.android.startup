...
public class pn_tr_for_so_issue_mgr extends pn_mgr {

	public pn_tr_for_so_issue_mgr(Context context) {
		super(context);
	}
    ...
    public void setContentView()
	{
        ...
        View view = App.Current.Workbench.getLayoutInflater().inflate(R.layout.pn_tr_for_so_issue_mgr, this, true);
	...
	public void create()
	{
		...
		if (this.Adapter.DataTable != null && this.Adapter.DataTable.Rows.size() > 0) {
			...
			Link link = new Link("pane://x:code=mm_and_tr_for_so_issue_editor");
...
//	        this.getContext().startActivity(intent);
			...
			Result<DataTable> r = App.Current.DbPortal.ExecuteDataTable(pn_tr_for_so_issue_mgr.this.Connector, sql, p);
			if (r.HasError) {
				App.Current.toastInfo(pn_tr_for_so_issue_mgr.this.getContext(), r.Error);
			}
			
			if (r.Value != null && r.Value.Rows.size() > 0) {
				App.Current.toastInfo(pn_tr_for_so_issue_mgr.this.getContext(), "转库申请【" + code + "】还有" + String.valueOf(r.Value.Rows.size()) + "条记录没有发完，不能提交。");
			} else {
				this.txt_locations = new EditText(pn_tr_for_so_issue_mgr.this.getContext());
				new AlertDialog.Builder(pn_tr_for_so_issue_mgr.this.getContext())
				.setTitle("请扫描储位")
				...
				.setPositiveButton("确定", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if (locations.length() > 0) {
							...
							Result<Integer> r = App.Current.DbPortal.ExecuteNonQuery(pn_tr_for_so_issue_mgr.this.Connector, sql, p);
							if (r.HasError) {
								App.Current.toastInfo(pn_tr_for_so_issue_mgr.this.getContext(), r.Error);
							}
							
							if (r.Value > 0) {
								App.Current.toastInfo(pn_tr_for_so_issue_mgr.this.getContext(), "提交成功!");
								pn_tr_for_so_issue_mgr.this.refresh();
							}
							txt_locations.setText("");
						} else {
							App.Current.showInfo(pn_tr_for_so_issue_mgr.this.getContext(), "请先扫描或输入储位。");
						}
					}
