...
public class pn_qm_smt_rejects_record_parameter_mgr extends pn_mgr {
    private SharedPreferences sharedPreferences;
    private SharedPreferences.Editor edit;

    public pn_qm_smt_rejects_record_parameter_mgr(Context context) {
        super(context);
    } @Override
    public void setContentView() {
        super.setContentView();
        sharedPreferences = App.Current.Workbench.getSharedPreferences("smt", Context.MODE_PRIVATE);
...
//        txt_line5.setText(row.getValue("items", ""));
    ...
    public void openItem(DataRow row) {
        super.openItem(row);
        Link link = new Link("pane://x:code=qm_smt_rejects_record_mgr");
