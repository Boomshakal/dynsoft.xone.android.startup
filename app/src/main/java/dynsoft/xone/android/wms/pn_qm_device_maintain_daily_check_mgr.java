...
public class pn_qm_device_maintain_daily_check_mgr extends pn_mgr {
    public pn_qm_device_maintain_daily_check_mgr(Context context) {
        super(context);
    }
    ...
    public View getItemView(int position, View convertView, ViewGroup parent) {
        ...
        String ipqc_code = row.getValue("ipqc_code", "");
        Date date = new Date(0);
        Date create_date = row.getValue("create_date", date);
        txt_line1.setText("线别：" + work_line);
        txt_line2.setText("责任人：" + respond_by);
        txt_line3.setText("IPQC：" + ipqc_code);
    ...
    public void create() {
        super.create();
        Link link = new Link("pane://x:code=qm_device_maintain_daily_check_mgr_editor");
    ...
    public void openItem(DataRow row) {
        ...
        link.Parameters.add("device_type", row.getValue("device_type"));
    ...
    public SqlExpression getSqlExpression(int top, int start, int end, String search) {
        ...
        expr.SQL = "exec p_qm_device_maintain_daily_check_get_items ?,?,?,?";
