...
public class pn_so_after_sales_scanning extends pn_editor implements OnClickListener {

    public pn_so_after_sales_scanning(Context context) {
        super(context);
    }
    ...
    public void loadItem(String code) {
        ...
        App.Current.DbPortal.ExecuteRecordAsync(this.Connector, sql, p, new ResultHandler<DataRow>() {
            ...
            public void handleMessage(Message msg) {
                pn_so_after_sales_scanning.this.ProgressDialog.dismiss();

                Result<DataRow> result = this.Value;
                if (result.HasError) {
                    App.Current.showError(pn_so_after_sales_scanning.this.getContext(), result.Error);
                    return;
                }

                DataRow row = result.Value;
                if (row == null) {
                    App.Current.showError(pn_so_after_sales_scanning.this.getContext(), "找不到该物料的对应的信息");

                    pn_so_after_sales_scanning.this.Header.setTitleText("找不到该物料的对应的信息");
                    clear();
                    return;
                }
                ...
                pn_so_after_sales_scanning.this.txt_customer_name_cell.setContentText(row.getValue("customer_name", ""));
                pn_so_after_sales_scanning.this.txt_item_name_cell.setContentText(row.getValue("item_name", ""));
                pn_so_after_sales_scanning.this.txt_item_code_cell.setContentText(row.getValue("item_code", ""));
                pn_so_after_sales_scanning.this.txt_sn_number_cell.setContentText(row.getValue("sn_number", ""));
                pn_so_after_sales_scanning.this.txt_code_cell.setContentText(row.getValue("code", ""));
        ...
        App.Current.DbPortal.ExecuteDataTableAsync(this.Connector, sql1, p1, new ResultHandler<DataTable>() {
            ...
            public void handleMessage(Message msg) {
                ...
                if (result.Value == null) {
                    App.Current.showError(pn_so_after_sales_scanning.this.getContext(), "系统没有找到该物料的待检记录");

                    pn_so_after_sales_scanning.this.Header.setTitleText("系统没有找到该物料的待检记录");
                    clear();
                    return;
                }

                myAdapter = new MyAdapter(result.Value);
                pn_so_after_sales_scanning.this.txt_item_main_cell.setAdapter(myAdapter);
				/*pn_so_after_sales_batch_show.this.txt_item_code_cell.setContentText(row.getValue("item_code", ""));
				pn_so_after_sales_batch_show.this.txt_item_name_cell.setContentText(row.getValue("item_name", ""));*/
    ...
    public void commit() {
        ...
        if (TextUtils.isEmpty(sn_number)) {
            App.Current.showError(pn_so_after_sales_scanning.this.getContext(), "请扫描物料");
            pn_so_after_sales_scanning.this.Header.setTitleText("请扫描物料");
            return;
        }
        ...
        App.Current.DbPortal.ExecuteDataTableAsync(this.Connector, sql, p, new ResultHandler<DataTable>() {
            ...
            public void handleMessage(Message msg) {
                ...
                if (result.Value == null) {
                    App.Current.showError(pn_so_after_sales_scanning.this.getContext(), "没有返回物料");

                    pn_so_after_sales_scanning.this.Header.setTitleText("没有返回物料");
                    clear();
                    return;
                }
                ...
                pn_so_after_sales_scanning.this.ProgressDialog.dismiss();
                if (rs1.equals("")) {
                    App.Current.toastInfo(pn_so_after_sales_scanning.this.getContext(), "该批次已经全部检查");
                    clear();
                    return;
                } else {
                    App.Current.toastInfo(pn_so_after_sales_scanning.this.getContext(), "物料完成检查");
                }
                clear();
            }
        });
        ...
        if (!(rs1.equals(""))) {
            ...
            App.Current.DbPortal.ExecuteDataTableAsync(this.Connector, sql3, p3, new ResultHandler<DataTable>() {
                ...
                public void handleMessage(Message msg) {
                    ...
                    if (result.Value == null) {
                        App.Current.showError(pn_so_after_sales_scanning.this.getContext(), "找不到该料号的对应的信息");

                        pn_so_after_sales_scanning.this.Header.setTitleText("找不到该料号号的对应的信息");
                        clear();
                        return;
                    }
                    myAdapter =new MyAdapter(result.Value);
