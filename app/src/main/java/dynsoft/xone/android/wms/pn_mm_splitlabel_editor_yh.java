...
public class pn_mm_splitlabel_editor_yh extends pn_editor {

    public pn_mm_splitlabel_editor_yh(Context context) {
        super(context);
    }
    ...
    public void loadLotNumber(String lotNumber) {
        ...
        String batchlot = pn_mm_splitlabel_editor_yh.this.txt_lot_number_cell.getContentText().trim();
        String vendorno = "";
        if (lot.length > 0) {
            int l = lot[0].length();
            vendorno = lot[0].substring(3, l - 10);
        }
        pn_mm_splitlabel_editor_yh.this.txt_vendor_no_cell.setContentText(vendorno);
        pn_mm_splitlabel_editor_yh.this.txt_quantity_cell.setContentText(lot[2]
                .toString());
        pn_mm_splitlabel_editor_yh.this.txt_date_code_cell.setContentText(lot[3]
                .toString());
        pn_mm_splitlabel_editor_yh.this.txt_item_code_cell.setContentText(lot[1]
                .toString());
            ...
            DialogInterface.OnClickListener listener = new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    if (which >= 0) {
                        ...
                        pn_mm_splitlabel_editor_yh.this.txt_org_code_cell.setContentText(row.getValue(
                                "code", ""));
                    }
                    dialog.dismiss();
                }
            };
            new AlertDialog.Builder(pn_mm_splitlabel_editor_yh.this.getContext()).setTitle("选择组织")
                    .setSingleChoiceItems(names.toArray(new String[0]), names.indexOf(pn_mm_splitlabel_editor_yh.this.txt_org_code_cell.getContentText().toString()), listener)
    ...
    public void loadVendorInfo() {
        ...
        if (result.Value != null && result.Value.Rows.size() > 0) {
            if (result.Value.Rows.size() > 1) {
                ...
                new AlertDialog.Builder(pn_mm_splitlabel_editor_yh.this.getContext())
                        .setTitle("选择厂商")
                        ...
                        if (i == 1) {
                            ...
                            App.Current.DbPortal.ExecuteRecordAsync("core_and", sql, new ResultHandler<DataRow>() {
                                ...
                                public void handleMessage(Message msg) {
                                    ...
                                    if (value.HasError) {
                                        App.Current.toastError(pn_mm_splitlabel_editor_yh.this.getContext(), value.Error);
                                        return;
                                    }
                                    if (value.Value != null) {
