...
public class feeder_maintain_mgr extends pn_editor {

	public feeder_maintain_mgr(Context context) {
		super(context);
	}
	...
	public void setContentView() {
		...
		View view = App.Current.Workbench.getLayoutInflater().inflate(
				R.layout.feeder_maintain_editor, this, true);
		view.setLayoutParams(lp);
	}
	
	  ArrayList<Integer>MultiChoiceID = new ArrayList<Integer>(); 
	  final String [] nItems = {"保养（拧紧螺丝、清洁加油、校正中心）","更换卷料轮及弹簧","更换叼咀及叼咀弹簧","更换手柄","更换气缸","更换定位柱","更换压料盖及压料盖配件","更换送料拉杆及拉杆弹簧","更换马达"};
	...
	public void onPrepared() {
		...
		this.txt_present_date_cell = (TextCell) this
				.findViewById(R.id.txt_present_date_cell);
		this.txt_present_type_cell = (TextCell) this
				.findViewById(R.id.txt_present_type_cell);
		
		this.txt_present_user_cell = (TextCell) this
				.findViewById(R.id.txt_present_user_cell);
		...
		if (this.txt_present_date_cell != null) {
			this.txt_present_date_cell.setLabelText("送修日期");
			this.txt_present_date_cell.setReadOnly();
		}
		
		if (this.txt_present_type_cell != null) {
			this.txt_present_type_cell.setLabelText("送修类型");
			this.txt_present_type_cell.setReadOnly();
		}
		if (this.txt_present_user_cell != null) {
			this.txt_present_user_cell.setLabelText("送修人员");
			this.txt_present_user_cell.setReadOnly();
		}
		
		if (this.txt_maintain_date_cell != null) {
			this.txt_maintain_date_cell.setLabelText("维修日期");
			this.txt_maintain_date_cell.setReadOnly();
		}
		 
		
		if (this.txt_maintain_user_cell != null) {
			this.txt_maintain_user_cell.setLabelText("维修人员");
		...
		if (this.txt_maintain_type_cell != null) {
			this.txt_maintain_type_cell.setLabelText("维修类型");
			this.txt_maintain_type_cell.Button.setOnClickListener(new OnClickListener() {  
	              
		            @Override  
		            public void onClick(View v) {
		            	//load_repair_type();
		                AlertDialog.Builder builder = new AlertDialog.Builder(feeder_maintain_mgr.this.getContext());
		                MultiChoiceID.clear();
		                ...
		                builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {  
		                    ...
		                    public void onClick(DialogInterface arg0, int arg1) {  
		                        ...
		                        feeder_maintain_mgr.this.txt_maintain_type_cell.setContentText(str) ;
		                    }  
		                });  
		                //  设置取消按钮  
	...
	public void loadItem(String code)
	{
		...
		String sql ="SELECT code,ng_type,repair_userinfo,present_datetime,valid_date FROM fm_smt_feeder where code = ? and is_repair=1";
		Parameters p  =new Parameters().add(1, code);
		App.Current.DbPortal.ExecuteRecordAsync(this.Connector, sql, p, new ResultHandler<DataRow>(){
			@Override
    	    public void handleMessage(Message msg) {
				feeder_maintain_mgr.this.ProgressDialog.dismiss();
				
				Result<DataRow> result = this.Value;
				if (result.HasError) {
					App.Current.showError(feeder_maintain_mgr.this.getContext(), result.Error);
					return;
				}
				
				DataRow row = result.Value;
				if (row == null) {
					App.Current.showError(feeder_maintain_mgr.this.getContext(), "找不到此供料器的送修信息");
					
					feeder_maintain_mgr.this.Header.setTitleText("找不到此供料器送修信息");
					return;
				}			
				feeder_maintain_mgr.this.txt_feeder_code_cell.setContentText(row.getValue("code", ""));
				feeder_maintain_mgr.this.txt_present_type_cell.setContentText(row.getValue("ng_type", ""));
				feeder_maintain_mgr.this.txt_present_user_cell.setContentText(row.getValue("repair_userinfo", ""));
				feeder_maintain_mgr.this.txt_present_date_cell.setContentText(row.getValue("present_datetime", new Date()).toString());
				feeder_maintain_mgr.this.txt_maintain_date_cell.setContentText(row.getValue("present_datetime", new Date()).toString());
				...
				feeder_maintain_mgr.this.txt_valid_date_cell.setContentText(row.getValue("valid_date",tomorrow).toString());
	...
	public void commit() {
		...
		String sql = " exec fm_smt_feeder_maintain_create ?,?,?,?,?,?,?,? ";		
		...
		Parameters p = new Parameters();
		 p.add(1, feeder_code).add(2, maintain_date).add(3, valid_date).add(4, maintain_type).add(5, maintain_user).add(6, present_type).add(7, present_user).add(8, present_date);
		App.Current.DbPortal.ExecuteDataTableAsync(this.Connector, sql, p, new ResultHandler<DataTable>() {
			...
			public void handleMessage(Message msg) {
				...
				App.Current.toastInfo(feeder_maintain_mgr.this.getContext(), "维修成功");
