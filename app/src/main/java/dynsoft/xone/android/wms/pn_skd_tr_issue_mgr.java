...
public class pn_skd_tr_issue_mgr extends pn_mgr {

    public pn_skd_tr_issue_mgr(Context context) {
        super(context);
    }
    ...
    public void create()
    {
        ...
        if (this.Adapter.DataTable != null && this.Adapter.DataTable.Rows.size() > 0) {
            ...
            Link link = new Link("pane://x:code=mm_and_skd_tr_issue_editor");
    ...
    public void openItem(DataRow row)
    {
        ...
        link.Parameters.add("order_id", row.getValue("id", ""));
        link.Parameters.add("order_code", row.getValue("code", ""));
        link.Parameters.add("line_id", row.getValue("pack_id", ""));
        link.Open(this, this.getContext(), null);
    }
                    ...
                    .setItems(items, new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialogInterface, int i) {
                            if (i == 1) {
                                ...
                                String sql = "exec p_mm_win_skd_tr_order_get_report_data ?,?,?";
                                ...
                                App.Current.DbPortal.ExecuteDataSetAsync("core_and", sql, p, new ResultHandler<DataSet>() {
                                    ...
                                    public void handleMessage(Message msg) {
                                        ...
                                        if (value.HasError) {
                                            App.Current.toastError(pn_skd_tr_issue_mgr.this.getContext(), value.Error);
                                            return;
                                        }
...
//	        this.getContext().startActivity(intent);
            ...
            String sql = "exec p_mm_skd_tr_issue_get_uncommited_items ?,?,?";
            Parameters p = new Parameters().add(1, App.Current.UserID).add(2, id).add(3, line_id);
            Result<DataTable> r = App.Current.DbPortal.ExecuteDataTable(pn_skd_tr_issue_mgr.this.Connector, sql, p);
            if (r.HasError) {
                App.Current.toastInfo(pn_skd_tr_issue_mgr.this.getContext(), r.Error);
            }

            if (r.Value != null && r.Value.Rows.size() > 0) {
                App.Current.toastInfo(pn_skd_tr_issue_mgr.this.getContext(), "转库申请【" + code + "】还有" + String.valueOf(r.Value.Rows.size()) + "条记录没有发完，不能提交。");
            } else {
                this.txt_locations = new EditText(pn_skd_tr_issue_mgr.this.getContext());
                new AlertDialog.Builder(pn_skd_tr_issue_mgr.this.getContext())
                        .setTitle("请扫描储位")
                        ...
                        .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                if (locations.length() > 0) {
                                    String sql = "exec p_mm_skd_tr_issue_commit ?,?,?,?";
                                    Parameters p = new Parameters().add(1, App.Current.UserID).add(2, id).add(3, line_id).add(4, locations);
                                    Result<Integer> r = App.Current.DbPortal.ExecuteNonQuery(pn_skd_tr_issue_mgr.this.Connector, sql, p);
                                    if (r.HasError) {
                                        App.Current.toastInfo(pn_skd_tr_issue_mgr.this.getContext(), r.Error);
                                    }

                                    if (r.Value > 0) {
                                        App.Current.toastInfo(pn_skd_tr_issue_mgr.this.getContext(), "提交成功!");
                                        pn_skd_tr_issue_mgr.this.refresh();
                                    }
                                    txt_locations.setText("");
                                } else {
                                    App.Current.showInfo(pn_skd_tr_issue_mgr.this.getContext(), "请先扫描或输入储位。");
                                }
                            }
    ...
    public View getItemView(int position, View convertView, ViewGroup parent)
    {
        ...
        txt_tr_order_code.setText("任务单号："+row.getValue("shipment_num", ""));

        String os_no = row.getValue("os_no", "");
        txt_item_code.setText("订单号："+os_no);
        txt_item_name.setText("批次号："+row.getValue("pack_number", ""));

        Integer vendor_name = row.getValue("box_number", 0);
        txt_vendor_name.setText("箱号："+ String.valueOf(vendor_name));
...
//        }
    ...
    public SqlExpression getSqlExpression(int top, int start, int end, String search)
    {
        ...
        expr.SQL = "exec p_mm_skd_tr_issue_get_items ?,?,?,?";
