...
public class pn_up_issue_mgr extends pn_mgr {

    public pn_up_issue_mgr(Context context) {
        super(context);
    }
    ...
    public void setContentView() {
        ...
        View view = App.Current.Workbench.getLayoutInflater().inflate(R.layout.pn_up_issue_mgr, this, true);
                            ...
                            if (i == 1) {
                                ...
                                App.Current.DbPortal.ExecuteDataSetAsync("core_and", sql, p, new ResultHandler<DataSet>() {
                                    ...
                                    public void handleMessage(Message msg) {
                                        ...
                                        if (value.HasError) {
                                            App.Current.toastError(pn_up_issue_mgr.this.getContext(), value.Error);
                                            return;
                                        }
...
//                                App.Current.Workbench.startActivity(intent);
            ...
            Result<DataTable> r = App.Current.DbPortal.ExecuteDataTable(pn_up_issue_mgr.this.Connector, sql, p);
            if (r.HasError) {
                App.Current.toastInfo(pn_up_issue_mgr.this.getContext(), r.Error);
            }

            if (r.Value != null && r.Value.Rows.size() > 0) {
                App.Current.toastInfo(pn_up_issue_mgr.this.getContext(), "发料申请【" + code + "】还有" + String.valueOf(r.Value.Rows.size()) + "条记录没有发完，不能提交。");
            } else {
                this.txt_locations = new EditText(pn_up_issue_mgr.this.getContext());
                new AlertDialog.Builder(pn_up_issue_mgr.this.getContext())
                        .setTitle("请扫描储位")
                        ...
                        .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                if (locations.length() > 0) {
                                    ...
                                    App.Current.DbPortal.ExecuteNonQueryAsync(pn_up_issue_mgr.this.Connector, sql, p, new ResultHandler<Integer>() {
                                        @Override
                                        public void handleMessage(Message msg) {
                                            Result<Integer> value = Value;
                                            if (value.HasError) {
                                                App.Current.toastInfo(pn_up_issue_mgr.this.getContext(), value.Error);
                                            } else {
                                                if (value.Value != null && value.Value > 0) {
                                                    App.Current.toastInfo(pn_up_issue_mgr.this.getContext(), "提交成功!");
                                                    pn_up_issue_mgr.this.refresh();
                                                } else {
                                                    App.Current.toastInfo(pn_up_issue_mgr.this.getContext(), "提交失败!");
                                                }
...
//							txt_locations.setText("");
                                    ...
                                    App.Current.showInfo(pn_up_issue_mgr.this.getContext(), "请先扫描或输入储位。");
                                }
                            }
