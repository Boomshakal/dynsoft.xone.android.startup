...
public class pn_tr_transaction_editor extends pn_editor {

	public pn_tr_transaction_editor(Context context) {
		super(context);
	}
	...
	public void onPrepared() {
		...
		if (this.txt_issue_quantity_cell != null) {
			...
			this.txt_issue_quantity_cell.TextBox.addTextChangedListener(new TextWatcher(){
				...
				public void afterTextChanged(Editable s) {
					DataRow row = (DataRow)pn_tr_transaction_editor.this.txt_onhand_cell.getTag();
					if (row != null) {
						...
						pn_tr_transaction_editor.this.txt_surplus_cell.setContentText(App.formatNumber(surplus, "0.##"));
					}
				}
			...
			this.txt_issue_quantity_cell.Button.setOnClickListener(new OnClickListener(){
				...
				public void onClick(View v) {
					String qty = pn_tr_transaction_editor.this.txt_issue_quantity_cell.getContentText();
					pn_tr_transaction_editor.this.printLabel(qty);
				}
			});
		}
		...
		if (this.txt_surplus_cell != null) {
			...
			this.txt_surplus_cell.Button.setOnClickListener(new OnClickListener(){
				@Override
				public void onClick(View v) {
					String qty = pn_tr_transaction_editor.this.txt_surplus_cell.getContentText();
					pn_tr_transaction_editor.this.printLabel(qty);
				}
			});
		}
		...
		if (this.btn_prev != null) {
			...
			this.btn_prev.setOnClickListener(new OnClickListener(){
				@Override
				public void onClick(View v) {
					pn_tr_transaction_editor.this.prev();
				}
			});
		}
		...
		if (this.btn_next != null) {
			...
			this.btn_next.setOnClickListener(new OnClickListener(){
				@Override
				public void onClick(View v) {
					pn_tr_transaction_editor.this.next();
				}
			});
		}
		...
		if (_line_id > 0) {
			...
			if (r.HasError) {
				App.Current.showError(pn_tr_transaction_editor.this.getContext(), "没有指定转移申请编号。");
				this.close();
			}
	...
	public void prev()
	{
		if (_rownum >1 ) {
			...
			App.Current.showError(pn_tr_transaction_editor.this.getContext(), "已经是第一条。");
		}
	}
	...
	public void next()
	{
		if (_rownum < _total) {
			...
			App.Current.showError(pn_tr_transaction_editor.this.getContext(), "已经是最后一条。");
		}
	}
	...
	public void loadTaskOrderItem(long index)
	{
		...
		if (code == null || code.length() == 0) {
			App.Current.showError(pn_tr_transaction_editor.this.getContext(), "没有指定转出申请编号。");
			return;
		}
		...
		App.Current.DbPortal.ExecuteRecordAsync(this.Connector, sql, p, new ResultHandler<DataRow>(){
    	    ...
    	    public void handleMessage(Message msg) {
				pn_tr_transaction_editor.this.ProgressDialog.dismiss();
				
				Result<DataRow> result = this.Value;
				if (result.HasError) {
					App.Current.showError(pn_tr_transaction_editor.this.getContext(), result.Error);
					return;
				}
				
				DataRow row = result.Value;
				if (row == null) {
					App.Current.showError(pn_tr_transaction_editor.this.getContext(), "没有数据。");
					pn_tr_transaction_editor.this.clearAll();
					pn_tr_transaction_editor.this.Header.setTitleText("库存转出(无转出任务)");
					return;
				}
				...
				if (_total > 0 ){
					pn_tr_transaction_editor.this.Header.setTitleText("库存转出(" + String.valueOf(_total) + "条转出任务)");
				} else {
					pn_tr_transaction_editor.this.Header.setTitleText("库存转出(无转出任务)");
				}
				
				pn_tr_transaction_editor.this.txt_tr_order_cell.setTag(row);
				
				String item_code = row.getValue("item_code", "") + ", 第" + String.valueOf(_rownum) + "条";
				pn_tr_transaction_editor.this.txt_tr_order_cell.setContentText(row.getValue("code", ""));
				pn_tr_transaction_editor.this.txt_item_code_cell.setContentText(item_code);
				pn_tr_transaction_editor.this.txt_item_name_cell.setContentText(row.getValue("item_name", ""));
				pn_tr_transaction_editor.this.txt_vendor_name_cell.setContentText(row.getValue("vendor_name", ""));
				pn_tr_transaction_editor.this.txt_date_code_cell.setContentText(row.getValue("date_code", ""));
				...
				pn_tr_transaction_editor.this.txt_quantity_cell.setContentText(qty_str);
				pn_tr_transaction_editor.this.txt_location_cell.setContentText(row.getValue("location", ""));
				
				pn_tr_transaction_editor.this.txt_lot_number_cell.setTag(null);
				pn_tr_transaction_editor.this.txt_lot_number_cell.setContentText("");
				pn_tr_transaction_editor.this.txt_issue_quantity_cell.setContentText("");
				pn_tr_transaction_editor.this.txt_onhand_cell.setContentText("");
				pn_tr_transaction_editor.this.txt_onhand_cell.setTag(null);
				pn_tr_transaction_editor.this.txt_surplus_cell.setContentText("");
			}
		});
	}
	...
	public void loadLotNumber(String lotNumber)
	{
		...
		App.Current.DbPortal.ExecuteRecordAsync(this.Connector, sql, p, new ResultHandler<DataRow>(){
			...
			public void handleMessage(Message msg) {
				...
				if (r.HasError) {
					App.Current.showError(pn_tr_transaction_editor.this.getContext(), r.Error);
					return;
				}
				
				String result = r.Value.getValue("result", "");
				if (result.length()>0) {
					App.Current.showError(pn_tr_transaction_editor.this.getContext(), result);
					return;
				}
				...
				pn_tr_transaction_editor.this.txt_vendor_name_cell.setContentText(r.Value.getValue("vendor_name", ""));
				pn_tr_transaction_editor.this.txt_vendor_name_cell.setTag(r.Value.getValue("vendor_id"));
				pn_tr_transaction_editor.this.txt_lot_number_cell.setTag(r.Value);
				pn_tr_transaction_editor.this.txt_lot_number_cell.setContentText(r.Value.getValue("lot_number", ""));
				pn_tr_transaction_editor.this.txt_date_code_cell.setContentText(r.Value.getValue("date_code", ""));
				pn_tr_transaction_editor.this.txt_location_cell.setContentText(r.Value.getValue("locations", ""));
				
				pn_tr_transaction_editor.this.txt_onhand_cell.setContentText(onhand_qty);
				pn_tr_transaction_editor.this.txt_onhand_cell.setTag(r.Value);
				pn_tr_transaction_editor.this.txt_surplus_cell.setContentText(surplus_qty);
				
				if (_scan_quantity == null || _scan_quantity.length() == 0) {
					pn_tr_transaction_editor.this.txt_issue_quantity_cell.setContentText(issue_qty);
				} else {
					pn_tr_transaction_editor.this.txt_issue_quantity_cell.setContentText(_scan_quantity);
				}
			}
		});
	...
	public void commit()
	{
		...
		App.Current.question(this.getContext(), "确定要提交吗？", new DialogInterface.OnClickListener(){
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				Connection conn = App.Current.DbPortal.CreateConnection(pn_tr_transaction_editor.this.Connector);
				...
				try {
					...
					if (val != null) {
						...
						if (rs.HasError) {
							App.Current.showError(pn_tr_transaction_editor.this.getContext(), rs.Error);
							return;
						}
						
						App.Current.toastInfo(pn_tr_transaction_editor.this.getContext(), "提交成功");
						pn_tr_transaction_editor.this.clear();
						pn_tr_transaction_editor.this.loadTaskOrderItem(_rownum);
					}
				} catch (SQLException e) {
					App.Current.showInfo(pn_tr_transaction_editor.this.getContext(), e.getMessage());
					e.printStackTrace();
