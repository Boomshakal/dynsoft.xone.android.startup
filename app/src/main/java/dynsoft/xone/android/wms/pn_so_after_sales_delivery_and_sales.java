...
public class pn_so_after_sales_delivery_and_sales extends pn_editor implements OnClickListener {

    public pn_so_after_sales_delivery_and_sales(Context context) {
        super(context);
    }
    ...
    public void setContentView() {
        ...
        View view = App.Current.Workbench.getLayoutInflater().inflate(
                R.layout.pn_so_after_sales_delivery_and_sales, this, true);
    ...
    public void onPrepared() {
        ...
        resultDatatable1 = new DataTable();
    ...
    public void load() {
        String sql = "exec pn_so_after_sales_delivery_and_sales_select_3 ?";
        Log.e("len", "RESULT:::" + pn_so_after_sales_delivery_and_sales.this.code);
        String code1 = pn_so_after_sales_delivery_and_sales.this.code;
        ...
        App.Current.DbPortal.ExecuteDataTableAsync(this.Connector, sql, p, new ResultHandler<DataTable>() {
            ...
            public void handleMessage(Message msg) {
                pn_so_after_sales_delivery_and_sales.this.ProgressDialog.dismiss();

                Result<DataTable> result = this.Value;
                if (result.HasError) {
                    App.Current.showError(pn_so_after_sales_delivery_and_sales.this.getContext(), result.Error);
                    return;
                }
                DataTable row = result.Value;
                if (row == null) {
                    App.Current.showError(pn_so_after_sales_delivery_and_sales.this.getContext(), "该单待发物料为空");

                    pn_so_after_sales_delivery_and_sales.this.Header.setTitleText("该单待发物料为空");
                    clear();
                    return;
                }
                ...
                pn_so_after_sales_delivery_and_sales.this.txt_item_main_cell.setAdapter(pn_so_after_sales_delivery_and_sales.this.myAdapter);
                pn_so_after_sales_delivery_and_sales.this.txt_customer_name_cell.setContentText(pn_so_after_sales_delivery_and_sales.this.customer_name);
            }
        });
        String sqlsel = "exec pn_so_after_sales_and_sales_qel ?";
        Parameters psel = new Parameters();
        psel.add(1, pn_so_after_sales_delivery_and_sales.this.code);
        App.Current.DbPortal.ExecuteRecordAsync(this.Connector, sqlsel, psel, new ResultHandler<DataRow>() {
            public void handleMessage(Message msg) {
                pn_so_after_sales_delivery_and_sales.this.ProgressDialog.dismiss();
                ...
                pn_so_after_sales_delivery_and_sales.this.txt_quantity_cell.setContentText(String.valueOf(result.Value.getValue("quantity",0)));
            }
        });
    }
    ...
    public void onScan(final String barcode) {
        ...
        if (pn_so_after_sales_delivery_and_sales.this.resultDatatable1.Rows.size() > 0 ||
                pn_so_after_sales_delivery_and_sales.this.resultDatatable1 != null) {
            pn_so_after_sales_delivery_and_sales.this.resultDatatable1.Rows.clear();
        }
        if (bar_code.equals("")) {
            App.Current.showError(pn_so_after_sales_delivery_and_sales.this.getContext(), "没有填写sn编码");

            pn_so_after_sales_delivery_and_sales.this.Header.setTitleText("没有填写sn编码");
            return;
        } else if (bar_code.startsWith("CRQ:")) {
            if (bar_code.length() < 4) {
                App.Current.showError(pn_so_after_sales_delivery_and_sales.this.getContext(), "sn编码非法");

                pn_so_after_sales_delivery_and_sales.this.Header.setTitleText("sn编码非法");
                return;
            }
            String pc_number = bar_code.substring(4, 23);
            pn_so_after_sales_delivery_and_sales.this.txt_pc_number_cell.setContentText(pc_number.trim());
            loadItem1(pc_number);
        } else {
            pn_so_after_sales_delivery_and_sales.this.txt_sn_number_cell.setContentText(bar_code);
            loadItem(bar_code);
        }
    }
    ...
    public void loadItem(String code) {
        ...
        String sql1 = "exec pn_so_after_sales_delivery_and_sales_select_1 ?,?";//已经完成
        String code1 = pn_so_after_sales_delivery_and_sales.this.code;
        ...
        App.Current.DbPortal.ExecuteDataTableAsync(this.Connector, sql1, p1, new ResultHandler<DataTable>() {
            ...
            public void handleMessage(Message msg) {
                ...
                if (result.Value != null && result.Value.Rows.size() == 0) {
                    App.Current.showError(pn_so_after_sales_delivery_and_sales.this.getContext(), "系统找不到该sn编码物料");

                    pn_so_after_sales_delivery_and_sales.this.Header.setTitleText("系统找不到sn编码物料");
                    clear();
                    return;
                } else {
                    String item_code = result.Value.Rows.get(0).getValue("item_code").toString().trim();
                    pn_so_after_sales_delivery_and_sales.this.txt_item_code_cell.setContentText(item_code);
                    String item_name = result.Value.Rows.get(0).getValue("item_name").toString().trim();
                    pn_so_after_sales_delivery_and_sales.this.txt_item_name_cell.setContentText(item_name);
                    String customer_name = result.Value.Rows.get(0).getValue("customer_name").toString().trim();
                    pn_so_after_sales_delivery_and_sales.this.txt_customer_name_cell.setContentText(customer_name);
                    int quantity = result.Value.Rows.get(0).getValue("quantity", 0);
                    pn_so_after_sales_delivery_and_sales.this.txt_quantity_cell.setContentText(String.valueOf(quantity));
                    String storage_number = result.Value.Rows.get(0).getValue("storage_number").toString().trim();
                    pn_so_after_sales_delivery_and_sales.this.txt_storage_number_cell.setContentText(storage_number);
                    pn_so_after_sales_delivery_and_sales.this.sn_items_id = (Integer) result.Value.Rows.get(0).getValue("id");
                    pn_so_after_sales_delivery_and_sales.this.txt_pc_quantity_cell.setContentText("");
                }
            }
        });
    ...
    public void loadItem1(final String code) {
        ...
        App.Current.DbPortal.ExecuteRecordAsync(this.Connector, sql, p, new ResultHandler<DataRow>() {
            ...
            public void handleMessage(Message msg) {
                ...
                if (result.Value == null) {
                    App.Current.showError(pn_so_after_sales_delivery_and_sales.this.getContext(), "系统没有找到该批次");
                    pn_so_after_sales_delivery_and_sales.this.Header.setTitleText("系统没有找到该批次");
                    return;
                }
                ...
                /*if (result.Value.getValue("is_return","").trim().equals("NO")){
                    App.Current.showError(pn_so_after_sales_delivery_and_sales.this.getContext(), "该批次已登记不返还");
                    pn_so_after_sales_delivery_and_sales.this.Header.setTitleText("该批次已登记不返还");
                    return;
                }
                /*if (!(result.Value.getValue("customer_name","").trim().equals(customer_name))){
                    App.Current.showError(pn_so_after_sales_delivery_and_sales.this.getContext(), "批次客户不是发货客户");
                    pn_so_after_sales_delivery_and_sales.this.Header.setTitleText("批次客户不是发货客户");
                    return;
                }*/
                int pc_quantity = result.Value.getValue("quantity", 0);
                pn_so_after_sales_delivery_and_sales.this.txt_pc_quantity_cell.setContentText(String.valueOf(pc_quantity));
                pn_so_after_sales_delivery_and_sales.this.txt_storage_number_cell.setContentText(result.Value.getValue("storage_number", ""));
                pn_so_after_sales_delivery_and_sales.this.txt_customer_name_cell.setContentText(result.Value.getValue("customer_name", ""));
                pn_so_after_sales_delivery_and_sales.this.txt_item_name_cell.setContentText(result.Value.getValue("item_name", ""));
                pn_so_after_sales_delivery_and_sales.this.txt_item_code_cell.setContentText(result.Value.getValue("item_code", ""));
                int all_quantity = result.Value.getValue("all_quantity", 0);
                pn_so_after_sales_delivery_and_sales.this.txt_quantity_cell.setContentText(String.valueOf(all_quantity));
                pn_so_after_sales_delivery_and_sales.this.txt_comment_cell.setContentText(result.Value.getValue("comment", ""));
        ...
        App.Current.DbPortal.ExecuteDataTableAsync(this.Connector, sql1, p1, new ResultHandler<DataTable>() {
            ...
            public void handleMessage(Message msg) {
                ...
                //Log.e("len", "2RESULT:::" +String.valueOf(result.Value.Rows.get(0).getValue("sn_items_id","")));
                //Log.e("len", "RESULT:::" +String.valueOf(result.Value.Rows.size()));
                if (result.Value != null && result.Value.Rows.size() == 0) {
                    App.Current.showError(pn_so_after_sales_delivery_and_sales.this.getContext(), "该批次没有物料");
                    pn_so_after_sales_delivery_and_sales.this.Header.setTitleText("该批次没有物料");
                    clear();
                    pn_so_after_sales_delivery_and_sales.this.txt_pc_number_cell.setContentText("");
                    return;
                }
                if (result.Value == null) {
                    App.Current.showError(pn_so_after_sales_delivery_and_sales.this.getContext(), "系统没有查询到物料信息");
                    pn_so_after_sales_delivery_and_sales.this.Header.setTitleText("系统没有查询到物料信息");
                    clear();
                    pn_so_after_sales_delivery_and_sales.this.txt_pc_number_cell.setContentText("");
                    return;
                } else {
    ...
    class MyAdapter extends BaseAdapter {
        ...
        public View getView(int i, View view, ViewGroup viewGroup) {
            ...
            if (view == null) {
                view = View.inflate(getContext(), R.layout.delivery_and_sales_info, null);
    ...
    public void onClick(View view) {////////////1212121212///////////////这里没做
        switch (view.getId()) {
            case R.id.btn_all_delivery:

                AlertDialog.Builder builder = new AlertDialog.Builder(pn_so_after_sales_delivery_and_sales.this.getContext());
                builder.setTitle("确定结束发货吗");
                builder.setMessage("确定结束发货吗？");
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        App.Current.DbPortal.ExecuteNonQueryAsync("core_and", sqldel, pdel, new ResultHandler<Integer>() {
                            ...
                            public void handleMessage(Message msg) {
                                ...
                                App.Current.toastInfo(pn_so_after_sales_delivery_and_sales.this.getContext(), "已经结束该单发货。");
                                pn_so_after_sales_delivery_and_sales.this.txt_sn_number_cell.setContentText("");
                                pn_so_after_sales_delivery_and_sales.this.txt_item_code_cell.setContentText("");
                                pn_so_after_sales_delivery_and_sales.this.txt_item_name_cell.setContentText("");
                                pn_so_after_sales_delivery_and_sales.this.txt_customer_name_cell.setContentText("");
                                pn_so_after_sales_delivery_and_sales.this.txt_comment_cell.setContentText("");
                                pn_so_after_sales_delivery_and_sales.this.txt_storage_number_cell.setContentText("");
                                pn_so_after_sales_delivery_and_sales.this.txt_code_cell.setContentText("");
                                pn_so_after_sales_delivery_and_sales.this.txt_quantity_cell.setContentText("");
                                pn_so_after_sales_delivery_and_sales.this.txt_pc_quantity_cell.setContentText("");
                                txt_pc_number_cell.setVisibility(GONE);
                                txt_pc_quantity_cell.setVisibility(GONE);
                                resultDatatable.Rows.clear();
                                resultDatatable1.Rows.clear();
                                myAdapter = new MyAdapter(resultDatatable);
                                myAdapter.notifyDataSetChanged();
                                pn_so_after_sales_delivery_and_sales.this.txt_item_main_cell.setAdapter(myAdapter);
                                pn_so_after_sales_delivery_and_sales.this.code = "";
                            }
                        });
                    }
                });
    ...
    @Override
    public void commit() {
        if (resultDatatable1.Rows.size() > 0) {
            ...
            for (DataRow rw : resultDatatable1.Rows) {
                ...
                entry.put("comment", pn_so_after_sales_delivery_and_sales.this.txt_comment_cell.getContentText().trim());
                entry.put("code", pn_so_after_sales_delivery_and_sales.this.code);
                entry.put("pc_number", pn_so_after_sales_delivery_and_sales.this.txt_pc_number_cell.getContentText().trim());
                entries.add(entry);
            }
            String xml = XmlHelper.createXml("so_returns", null, null, "so_return", entries);
            Log.e("len", "result:::" + xml);
            String sqlb = "exec mm_after_delivery_head_add_sales_batch_PDA_20190114 ?";
            ...
            App.Current.DbPortal.ExecuteRecordAsync(this.Connector, sqlb, pb, new ResultHandler<DataRow>() {
                public void handleMessage(Message msg) {
                    ...
                    if (x == 0){
                        App.Current.toastInfo(pn_so_after_sales_delivery_and_sales.this.getContext(), "已经结束该单发货。");
                        exists();
                    }else{App.Current.toastInfo(pn_so_after_sales_delivery_and_sales.this.getContext(), "该批次物料已发货。");
                        pn_so_after_sales_delivery_and_sales.this.resultDatatable1.Rows.clear();
                        clear();
                        myAdapter.notifyDataSetChanged();
                        pn_so_after_sales_delivery_and_sales.this.txt_item_main_cell.setAdapter(myAdapter);
        ...
        String sql = "exec mm_after_delivery_head_add_sales_PDA ?,?,?,?";
        Parameters p = new Parameters();
        p.add(1, sn_number).add(2, comment).add(3, pn_so_after_sales_delivery_and_sales.this.sn_items_id)
                .add(4, pn_so_after_sales_delivery_and_sales.this.code);
        App.Current.DbPortal.ExecuteRecordAsync(this.Connector, sql, p, new ResultHandler<DataRow>() {
            @Override
            public void handleMessage(Message msg) {
                pn_so_after_sales_delivery_and_sales.this.ProgressDialog.dismiss();
                ...
                App.Current.toastInfo(pn_so_after_sales_delivery_and_sales.this.getContext(), "该物料已发货。");
                clear1();
                pn_so_after_sales_delivery_and_sales.this.txt_quantity_cell.setContentText(String.valueOf(result.Value.getValue("quantity",0)));
            }
        });
    ...
    public void exists(){
        String code1 = pn_so_after_sales_delivery_and_sales.this.code;
        String sqldel = "exec pn_so_after_sales_delivery_and_sales_finish ?";
        Parameters pdel = new Parameters().add(1, code1);
        App.Current.DbPortal.ExecuteNonQueryAsync("core_and", sqldel, pdel, new ResultHandler<Integer>() {
                    public void handleMessage(Message msg) {
                        Result<Integer> result = this.Value;
    ...
    public void clear() {
        ...
        String sql = "exec pn_so_after_sales_delivery_and_sales_select_3 ?";
        Parameters p = new Parameters().add(1, pn_so_after_sales_delivery_and_sales.this.code);
        App.Current.DbPortal.ExecuteDataTableAsync(this.Connector, sql, p, new ResultHandler<DataTable>() {
            @Override
            public void handleMessage(Message msg) {
                pn_so_after_sales_delivery_and_sales.this.ProgressDialog.dismiss();
                ...
                pn_so_after_sales_delivery_and_sales.this.myAdapter = new MyAdapter(result.Value);
                pn_so_after_sales_delivery_and_sales.this.txt_item_main_cell.setAdapter(myAdapter);
            }
        });
    }
