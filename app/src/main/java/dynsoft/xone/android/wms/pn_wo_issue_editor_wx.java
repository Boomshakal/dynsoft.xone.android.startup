...
public class pn_wo_issue_editor_wx extends pn_editor {

	public pn_wo_issue_editor_wx(Context context) {
		super(context);
	}
	...
	public void onPrepared() {
		...
		if (this.txt_wo_issue_order_cell != null) {
			...
			this.txt_wo_issue_order_cell.Button.setOnClickListener(new OnClickListener(){
				...
				public void onClick(View v) {
					pn_wo_issue_editor_wx.this.showList(false);
				}
			});
		}
		...
		if (this.txt_item_code_cell != null) {
			...
			this.txt_item_code_cell.Button.setOnClickListener(new OnClickListener(){
				@Override
				public void onClick(View v) {
					pn_wo_issue_editor_wx.this.showList(true);
				}
			});
		}
		...
		if (this.txt_lot_number_cell != null) {
			...
			this.txt_lot_number_cell.Button.setOnClickListener(new OnClickListener(){
				@Override
				public void onClick(View v) {
					String lot_number = pn_wo_issue_editor_wx.this.txt_lot_number_cell.getContentText().trim();
					if (lot_number == null || lot_number.length() == 0) {
						App.Current.showError(pn_wo_issue_editor_wx.this.getContext(), "没有指定批次。");
					} else {
						Link link = new Link("pane://x:code=mm_and_tr_move_editor");
						link.Parameters.add("lot_number", lot_number);
				        link.Open(pn_wo_issue_editor_wx.this, pn_wo_issue_editor_wx.this.getContext(), null);
					}
		...
		if (this.txt_issue_quantity_cell != null) {
			...
			this.txt_issue_quantity_cell.TextBox.addTextChangedListener(new TextWatcher(){
				...
				public void afterTextChanged(Editable s) {
					if (_lot_row != null) {
						...
						pn_wo_issue_editor_wx.this.txt_surplus_cell.setContentText(App.formatNumber(surplus, "0.##"));
					}
				}
			...
			this.txt_issue_quantity_cell.Button.setOnClickListener(new OnClickListener(){
				@Override
				public void onClick(View v) {
					String qty = pn_wo_issue_editor_wx.this.txt_issue_quantity_cell.getContentText();
					pn_wo_issue_editor_wx.this.printLabel(qty);
				}
			});
		}
		...
		if (this.txt_surplus_cell != null) {
			...
			this.txt_surplus_cell.Button.setOnClickListener(new OnClickListener(){
				@Override
				public void onClick(View v) {
					String qty = pn_wo_issue_editor_wx.this.txt_surplus_cell.getContentText();
					pn_wo_issue_editor_wx.this.printLabel(qty);
				}
			});
		}
		...
		if (this.btn_prev != null) {
			...
			this.btn_prev.setOnClickListener(new OnClickListener(){
				@Override
				public void onClick(View v) {
					pn_wo_issue_editor_wx.this.prev();
				}
			});
		}
		...
		if (this.btn_next != null) {
			...
			this.btn_next.setOnClickListener(new OnClickListener(){
				@Override
				public void onClick(View v) {
					pn_wo_issue_editor_wx.this.next();
				}
			});
		}
	...
	public void prev()
	{
		if (_rownum > 1) {
			...
			App.Current.showError(pn_wo_issue_editor_wx.this.getContext(), "已经是第一条。");
		}
	}
	...
	public void next()
	{
		if (_rownum < _total) {
			...
			App.Current.showError(pn_wo_issue_editor_wx.this.getContext(), "已经是最后一条。");
		}
	}
	...
	public void loadTaskOrderItem_x(long line_id)
	{
		this.ProgressDialog.show();
		
		String sql = "exec p_mm_wo_issue_get_order_item_wx ?,?,?";
		Parameters p  =new Parameters().add(1, App.Current.UserID).add(2, _order_id).add(3, line_id);
		App.Current.DbPortal.ExecuteRecordAsync(this.Connector, sql, p, new ResultHandler<DataRow>(){
			@Override
    	    public void handleMessage(Message msg) {
				pn_wo_issue_editor_wx.this.ProgressDialog.dismiss();
				
				Result<DataRow> result = this.Value;
				if (result.HasError) {
					App.Current.showError(pn_wo_issue_editor_wx.this.getContext(), result.Error);
					return;
				}
				
				DataRow row = result.Value;
				if (row == null) {
					App.Current.showError(pn_wo_issue_editor_wx.this.getContext(), "找不到发料任务。");
					pn_wo_issue_editor_wx.this.clearAll();
					pn_wo_issue_editor_wx.this.Header.setTitleText("工单发料(无发料任务)");
					return;
				}
				...
				if (_total > 0 ){
					pn_wo_issue_editor_wx.this.Header.setTitleText("工单发料(" + String.valueOf(_total) + "条发料任务)");
				} else {
					pn_wo_issue_editor_wx.this.Header.setTitleText("工单发料(无发料任务)");
				}
				
				pn_wo_issue_editor_wx.this.txt_wo_issue_order_cell.setTag(row);
				
				String item_code = row.getValue("item_code", "") + ", " + App.formatNumber(row.getValue("stock_quantity", ""), "0.##") + ", 第" + String.valueOf(_rownum) + "条";
				pn_wo_issue_editor_wx.this.txt_wo_issue_order_cell.setContentText(row.getValue("code", ""));
				pn_wo_issue_editor_wx.this.txt_item_code_cell.setContentText(item_code);
				pn_wo_issue_editor_wx.this.txt_item_name_cell.setContentText(row.getValue("item_name", ""));
				pn_wo_issue_editor_wx.this.txt_warehouse_cell.setContentText(row.getValue("warehouse_code", "") + ", " + row.getValue("locations", ""));
				pn_wo_issue_editor_wx.this.txt_vendor_name_cell.setContentText(row.getValue("vendor_name", ""));
				pn_wo_issue_editor_wx.this.txt_date_code_cell.setContentText(row.getValue("date_code", ""));
				...
				pn_wo_issue_editor_wx.this.txt_planned_quantity_cell.setContentText(qty_str);
				pn_wo_issue_editor_wx.this.txt_location_cell.setContentText(row.getValue("location", ""));
				
				pn_wo_issue_editor_wx.this.txt_lot_number_cell.setTag(null);
				pn_wo_issue_editor_wx.this.txt_lot_number_cell.setContentText("");
				pn_wo_issue_editor_wx.this.txt_issue_quantity_cell.setContentText("");
				pn_wo_issue_editor_wx.this.txt_onhand_cell.setContentText("");
				pn_wo_issue_editor_wx.this.txt_onhand_cell.setTag(null);
				pn_wo_issue_editor_wx.this.txt_surplus_cell.setContentText("");
				pn_wo_issue_editor_wx.this.loaditem(_lot_row,_showLotQuantity);
			}
		});
	}
                ...
                DialogInterface.OnClickListener listener = new DialogInterface.OnClickListener(){
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if (which >= 0) {
                        	...
                        	pn_wo_issue_editor_wx.this.showLotNumber(row, true);
                        }
                        dialog.dismiss();
                    }
                };
	...
	public void loadLotNumber(String lotNumber)
	{
		...
		String sql = "exec p_mm_wo_issue_get_lot_number_wx ?,?,?";
		...
		App.Current.DbPortal.ExecuteRecordAsync(this.Connector, sql, p, new ResultHandler<DataRow>(){
			...
			public void handleMessage(Message msg) {
				...
				if (r.HasError) {
					App.Current.showError(pn_wo_issue_editor_wx.this.getContext(), r.Error);
					return;
				   }	
				
				pn_wo_issue_editor_wx.this.showLotNumber(r.Value, false);
			}
		});
	}
	boolean _showLotQuantity;
	public void showLotNumber(DataRow row, boolean showLotQuantity)
	{
		_lot_row = row;
		final boolean  showLotQuantityv =showLotQuantity;
		_showLotQuantity =showLotQuantity;
		String result = _lot_row.getValue("result", "");
		if (result.length()>0) {
		   
			if (result.indexOf("\\^批次D/C")==-1)
			{
				App.Current.showError(pn_wo_issue_editor_wx.this.getContext(), result);
				return;
			}else
			{
				App.Current.question(pn_wo_issue_editor_wx.this.getContext(),result , new DialogInterface.OnClickListener(){
					@Override
					public void onClick(DialogInterface dialog, int which) {
						_order_id =_lot_row.getValue("order_id",0L);
						loadTaskOrderItem_x(_lot_row.getValue("line_id",0));
					}
				});
			}
	...
	public void loaditem(DataRow row, boolean showLotQuantity)
	{
		...
		pn_wo_issue_editor_wx.this.txt_vendor_name_cell.setContentText(_lot_row.getValue("vendor_name", ""));
		pn_wo_issue_editor_wx.this.txt_vendor_name_cell.setTag(_lot_row.getValue("vendor_id"));
		pn_wo_issue_editor_wx.this.txt_lot_number_cell.setContentText(_lot_row.getValue("lot_number", ""));
		pn_wo_issue_editor_wx.this.txt_date_code_cell.setContentText(_lot_row.getValue("date_code", ""));
		pn_wo_issue_editor_wx.this.txt_location_cell.setContentText(_lot_row.getValue("locations", ""));
		
		pn_wo_issue_editor_wx.this.txt_issue_quantity_cell.setContentText(issue_qty);
		pn_wo_issue_editor_wx.this.txt_onhand_cell.setContentText(onhand_qty);
		pn_wo_issue_editor_wx.this.txt_surplus_cell.setContentText(surplus_qty);
	}
	...
	public void commit()
	{
		...
		App.Current.question(this.getContext(), "确定要提交吗？", new DialogInterface.OnClickListener(){
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				Connection conn = App.Current.DbPortal.CreateConnection(pn_wo_issue_editor_wx.this.Connector);
				...
				try {
					...
					if (val != null) {
						...
						if (rs.HasError) {
							App.Current.showError(pn_wo_issue_editor_wx.this.getContext(), rs.Error);
							return;
						}
						
						App.Current.toastInfo(pn_wo_issue_editor_wx.this.getContext(), "提交成功");
						_order_row = null;
						_lot_row = null;
						pn_wo_issue_editor_wx.this.clear();
						pn_wo_issue_editor_wx.this.loadTaskOrderItem(_rownum);
					}
				} catch (SQLException e) {
					App.Current.showInfo(pn_wo_issue_editor_wx.this.getContext(), e.getMessage());
					e.printStackTrace();
