...
public class pn_smt_steel_car_back_editorr extends pn_editor {

	public pn_smt_steel_car_back_editorr(Context context) {
		super(context);
	}
	...
	public void setContentView() {
		...
		View view = App.Current.Workbench.getLayoutInflater().inflate(
				R.layout.pn_smt_steel_car_back_mgr, this, true);
	...
	private void toastChooseDialog1() {
		...
		new AlertDialog.Builder(pn_smt_steel_car_back_editorr.this.getContext()).setTitle("请选择")
	...
	public void commit() {
		...
		if (state == null || state.length() == 0) {
			...
			String sql = "exec mm_smt_steel_car_update_back ?,?,?,?,?,?";
			Parameters p = new Parameters();
//		Log.e("LZH","495");
//		Log.e("LZH",work_order_code_final);
			p.add(1, txt_item_code_cell.getContentText()).add(2, txt_store_code.getContentText()).add(3, txt_sate.getContentText()).add(4, txt_depart.getContentText()).add(5, txt_date_time.getContentText()).add(6, txt_usernumber_code_cell.getContentText());
			App.Current.DbPortal.ExecuteDataTableAsync(this.Connector, sql, p, new ResultHandler<DataTable>() {
				@Override
				public void handleMessage(Message msg) {
					pn_smt_steel_car_back_editorr.this.ProgressDialog.dismiss();
					App.Current.toastInfo(pn_smt_steel_car_back_editorr.this.getContext(), "周转车记录成功");
					clear();
	...
	public void  get_depart(){
		String sql = "SELECT TOP(1) * FROM dbo.mm_smt_steel_car_use_log where item_code = ? ORDER BY create_time DESC";
		Parameters p = new Parameters();
		p.add(1,txt_item_code_cell.getContentText());
		App.Current.DbPortal.ExecuteRecordAsync("core_and", sql,p,new ResultHandler<DataRow>() {

			public void handleMessage(Message msg) {
				Result<DataRow> value = Value;
				if(value.HasError){
					App.Current.showError(pn_smt_steel_car_back_editorr.this.getContext(), String.valueOf(value.Error));
				}else {
					flag = value.Value.getValue("is_store",0);
					String depart = value.Value.getValue("depart","");

					if(flag ==1){
						App.Current.showError(pn_smt_steel_car_back_editorr.this.getContext(), "此周转车已回库，请不要重复回库！");
						clear();
					}
					else{
						txt_depart.setContentText(depart);
					}
				}
			}
		});
	...
	//判断在不在库
//	public void  is_store(){
//		String sql = "SELECT TOP(1) * FROM dbo.mm_smt_steel_car_use_log where item_code = ? and is_store = 0 ORDER BY create_time DESC";
//		Parameters p = new Parameters();
//		p.add(1,txt_item_code_cell.getContentText());
//		App.Current.DbPortal.ExecuteRecordAsync("core_and", sql,p,new ResultHandler<DataRow>() {
//
