...
package dynsoft.xone.android.expression;
 ...
 */
public MathExpression(MathExpression oth) {
    super();
...
public MathExpression setConstant(String nam, double val) {
 ...
 */
public MathExpression setOperator(Operator opr) {
 ...
 */
public MathExpression setFunctionHandler(String nam, FunctionHandler hdl) {
 ...
 * Pure functions have results which depend purely on their arguments; given constant arguments they will have a constant result.  Impure functions are rare.
 */
public MathExpression setFunctionHandler(String nam, FunctionHandler hdl, boolean impure) {
    validateName(nam);
...
/** Set a named variable (variables names are not case-sensitive). */
public MathExpression setVariable(String nam, double val) {
...
public MathExpression setVariableRequired(boolean val) {
    relaxed=(!val);
    return this;
    }
...
// *************************************************************************************************
     ...
     * Operator Structure.
     * <p>
     * This class is immutable and threadsafe, but note that whether it can be used in multiple MathExpression instances (as
...
// *************************************************************************************************
    ...
    public double evaluateOperator(double lft, char opr, double rgt) {
        switch(opr) {
            ...
            default  : throw new UnsupportedOperationException("MathExpression internal operator setup is incorrect - internal operator \""+opr+"\" not handled");
            }
        }
...
//              if(fncnam.equalsIgnoreCase("nextUp"        )) { return Math.nextUp     (fncargs.next());                } needs Java 6
        ...
        throw new UnsupportedOperationException("MathExpression internal function setup is incorrect - internal function \""+fncnam+"\" not handled");
        }
    ...
    // To add/remove operators change evaluateOperator() and registration
    static void registerOperators(MathExpression tgt) {
    ...
    // To add/remove functions change evaluateOperator() and registration
    static void registerFunctions(MathExpression tgt) {
