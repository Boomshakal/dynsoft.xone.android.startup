...
public class DbSession {
	...
	private void PrepareParameters(CallableStatement statement, Parameters parameters) throws SQLException {
	...
	public Result<Integer> ExecuteNonQuery(String sql, Parameters parameters)
	{
        ...
        try {
        	if (_connection != null) {
        		if (parameters != null) {
        			CallableStatement stmt = _connection.prepareCall(sql);
	...
	public void ExecuteNonQueryAsync(final String sql, final Parameters parameters, final ResultHandler<Integer> handler)
	{
		new Thread(new Runnable(){
			...
			@Override
			public void run() {
				Result<Integer> r = DbSession.this.ExecuteNonQuery(sql, parameters);
				if (handler != null) {
					handler.Value = r;
	...
	public <T> void ExecuteScalarAsync(final String sql, final Parameters parameters, final Class<T> clazz, final ResultHandler<T> handler)
	{
		new Thread(new Runnable(){
			...
			@Override
			public void run() {
				Result<T> r = DbSession.this.ExecuteScalar(sql, parameters, clazz);
				if (handler != null) {
					handler.Value = r;
					handler.sendEmptyMessage(0);
				}
	...
	public void ExecuteRecordAsync(final String sql, final Parameters parameters, final ResultHandler<DataRow> handler)
	{
		new Thread(new Runnable(){
			...
			@Override
			public void run() {
				Result<DataRow> r = DbSession.this.ExecuteRecord(sql, parameters);
				if (handler != null) {
					handler.Value = r;
					handler.sendEmptyMessage(0);
				}
	...
	public Result<DataSet> ExecuteDataSet(String sql, Parameters parameters)
	{
		...
		try {
        	if (_connection != null) {
        		...
        		Statement stmt = null;
        		if (parameters != null) {
        			CallableStatement st = _connection.prepareCall(sql);
	...
	public void ExecuteDataSetAsync(final String sql, final Parameters parameters, final ResultHandler<DataSet> handler)
	{
		new Thread(new Runnable(){
			...
			@Override
			public void run() {
				Result<DataSet> r = DbSession.this.ExecuteDataSet(sql, parameters);
				if (handler != null) {
					handler.Value = r;
					handler.sendEmptyMessage(0);
				}
