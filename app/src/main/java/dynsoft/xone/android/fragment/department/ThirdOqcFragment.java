...
public class ThirdOqcFragment extends Fragment {
    ...
    private void initView() {
        ...
        String sql = "exec p_fm_get_third_department_and";
    ...
    class OqcAdapter extends BaseAdapter {
        ...
        public View getView(int i, View view, ViewGroup viewGroup) {
            ...
            //检验员
            final String tester = dataTable.Rows.get(i).getValue("tester", "");
            ...
            oqcViewHolder.textView2.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    //送检批
                    toastOqcQiantity(lineName, "", tester);
                }
            });
            if (unqualified_count > 0) {
                Log.e("len", "BADQIANTITY");
                badQiantity(lineName, "不合格", oqcViewHolder.textView5, tester);
            }
            oqcViewHolder.textView3.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    //不合格批
                    toastBadQiantity(lineName, "不合格", tester);
...
//            }
    ...
    private void badQiantity(String lineName, String status, final TextView textView, String tester) {
        String sql = "exec fm_third_oqc_task_item_datas ?,?,?";
        Log.e("len", lineName + "**(" + status + "**" + tester);
        Parameters p = new Parameters().add(1, lineName).add(2, status).add(3, tester);
    ...
    private void toastOqcQiantity(String lineName, String status, String tester) {
        progressBar.setVisibility(View.VISIBLE);
        String sql = "exec fm_third_oqc_task_item_datas ?,?,?";
        Parameters p = new Parameters().add(1, lineName).add(2, status).add(3, tester);
        App.Current.DbPortal.ExecuteDataTableAsync("core_and", sql, p, new ResultHandler<DataTable>() {
...
//                    popupWindow.setBackgroundDrawable(getResources().getDrawable(R.color.style_divider_color));
    ...
    private void toastBadQiantity(String workLine, String status, String tester) {
        String sql = "exec fm_third_oqc_task_item_datas ?,?,?";
        Parameters p = new Parameters().add(1, workLine).add(2, status).add(3, tester);
        App.Current.DbPortal.ExecuteDataTableAsync("core_and", sql, p, new ResultHandler<DataTable>() {
            @Override
            public void handleMessage(Message msg) {
                Result<DataTable> value = Value;
